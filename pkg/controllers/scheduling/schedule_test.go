...
func TestSchedule(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if len(result.feasibleClusters) != len(c.scheduleResult.feasibleClusters) {
				t.Errorf("expected %v feasible clusters, but got %v", c.scheduleResult.feasibleClusters, result.feasibleClusters)
			}
			if !reflect.DeepEqual(result.scheduledDecisions, c.scheduleResult.scheduledDecisions) {
				t.Errorf("expected %v scheduled, but got %v", c.scheduleResult.scheduledDecisions, result.scheduledDecisions)
			}
