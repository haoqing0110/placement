...
type scheduleResult struct {
	...
	scoreRecords    map[string]PrioritizeSore
}
...
func (s *pluginScheduler) Schedule(
	...
	for _, f := range s.filters {
		...
		results.filteredRecords[f.Name()] = filtered
	}
	...
	for _, p := range s.prioritizers {
		...
		results.scoreRecords[p.Name()] = score
...
func (r *scheduleResult) PriorizeResults() []PriorizeResult {
	results := []PriorizeResult{}
	for name, r := range r.scoreRecords {
		result := PriorizeResult{Name: name, Scores: r}
		results = append(results, result)
	}
